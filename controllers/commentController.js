...
exports.addComment = async (req, res, next) => {
  try {
    ...
    const comment = await Comment.create({ userId: req.user.id, workId, commentText });
...
exports.getCommentsForWork = async (req, res, next) => {
  try {
    ...
    const comments = await Comment.find({ workId }).sort({ createdAt: -1 });
    res.json(comments);
  } catch (err) {
    next(err);
  }
...
exports.getCommentsForPublisherWorks = async (req, res, next) => {
  try {
    ...
    const comments = await Comment.find({ workId: { $in: workIds } }).sort({ createdAt: -1 });
    res.json(comments);
  } catch (err) {
    next(err);
  }
